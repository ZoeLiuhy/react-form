...
const QuestionElement = ({ props, formData, setFormData, isValid }) => {
  const [inputValue, setInputValue] = useState(formData[props.name]);
  ...
  return (
    <Question>
      <QuestionTitle required={props.required}>{props.title}</QuestionTitle>
      <QuestionDescription showDescription={props.showDescription}>
        對活動的一些建議
      </QuestionDescription>
      <QuestionInput
      ...
      />
      {isValid[props.name] && <ErrorMessage>{props.errorMessage}</ErrorMessage>}
    </Question>
  );
};
/*
